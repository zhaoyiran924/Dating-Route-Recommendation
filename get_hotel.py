...
# def get_info(page):
#     print("in get_info")
#     names = re.findall(r'hotel_item_pic.*?title="(.*?)"',page,re.S)
#     locations = re.findall(r'<p class="hotel_item_htladdress">(.*?)<a href="javascript"',page,re.S)
#     for i in range(len(locations)):
#         if '】' in locations[i]:
#             locations[i] = locations[i].split('】')[1]
#     #scores = re.findall(r'"hotel_value">(.*?)</span>',page,re.S)
#     #prices = re.findall(r'<span class="J_price_lowlist">(.*?)</span>',page,re.S)
#     for name,location,score,price in zip(names,locations,scores,prices):
#         data = {}
#         data['name'] = name
#         data['score'] = score
#         data['price'] = price
...
def get_hotel():
    ...
    # urls = [basic_url + str(i) for i in range(1,10)]
    headers = {
    ...
    #get the html file and save as "unicode.txt"
    for i in range(1):
        url = basic_url
